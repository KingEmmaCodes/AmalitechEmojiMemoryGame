  ...
  setScores() {
    ...
    if (this.players == 1) {
      ...
      for (let i = 0; i < this.players; i++) {
        ...
        scores.innerHTML += `
				<div class="player${i + 1} score ${
          i == 0 ? "active" : ""
        }"><span><span class="player-name">Player</span> <span>${
          i + 1
        }</span></span><span class="player${
          i + 1
    ...
    if (this.currentMoves.length === 2) {
      ...
      if (this.currentMoves[0].value === this.currentMoves[1].value) {
        this.currentMoves.forEach((card) => {
          card.found = true;
          document
            .querySelector('[data-card-number="' + card.order + '"]')
            .classList.add("found");
        });
    ...
    if (this.board.filter((card) => card.found === false).length === 0) {
      ...
      if (this.players == 1) {
        ...
        popup.querySelector(".results").innerHTML = `
					<div class="result">time Elapsed <span class="time-elapsed font-big">${min}:${
          sec <= 9 ? "0" + sec : sec
        }</span></div>
					<div class="result">Moves Taken <span class="move-taken font-big">${
            this.move
          }</span></div>
					`;
      } else {
        ...
        for (let i = 0; i < this.score.length; i++) {
          ...
          finalScore += `
					<div class="player${this.score[i].player} ${
            this.score[i].winner ? "winner" : "looser"
          } score result">
  ...
  handleClick(e) {
    if (e.target.classList.contains("card")) {
      ...
      this.theme === "numbers"
        ...
        : (content = `<i class="fa ${icons[target.value - 1]}"></i>`);
  ...
  updateScores() {
    if (this.players == 1) {
      ...
      document.querySelector(
        ".player" + this.playerTurn + "-number"
      ).innerHTML = this.score[this.playerTurn - 1].value;
    }
  }
}
...
function clearGame() {
  document
    .querySelector(".board")
    .removeEventListener("click", game.boundEventHandler);
  if (game) {
    game.moves = null;
